...
<body>
 ...
 <!-- phan model -->
	...
	<div class="container tieude text-info">
    	...
    	<div><p><a href="dulichbien/trangchu.htm">Trang chủ</a> <i class="fas fa-angle-right"></i>Thanh toán</p></div>
   		 </div>
	</div>
    ...
    <div class="container mt-5 mb-5">
    	<div class="row">
        	<div class="col-md-8 col-sm-12 col-xs-12">
                ...
                <br>
                <p>Hãy điền vào những nội dung dưới đây và gửi cho chúng tôi, chúng tôi sẽ liên hệ với quý khách để tiến hành thanh toán.</p>
                <form:form action="dulichbien/xacnhan.htm" modelAttribute="khachhang" >
                	<div>
                    	<label><strong>Họ tên</strong></label>
                 	    <br>
               		     <form:input class="form-control" type="text" path="tenKH" placeholder="Họ tên(*)"/>
                    ...
                    <div class="mt-3">
                    	<label><strong>Số điện thoại</strong></label>
                 	    <br>
               		     <form:input class="form-control" type="text" path="sodienthoai" placeholder="Số điện thoại(*)"/>
               		     <form:errors path="Sodienthoai" style="color: red;font-style: italic;"></form:errors>
                    </div>
                     <div class="mt-3">
                    	<label><strong>Địa chỉ</strong></label>
                 	    <br>
               		     <form:input class="form-control" type="text" path="diachi" placeholder="Địa chỉ(*)"/>
               		     <form:errors path="Diachi" style="color: red;font-style: italic;"></form:errors>
                    </div>
                    <div class="mt-3">
                    	<label><strong>Nội dung</strong></label>
                 	    <br>
               		     <form:textarea class="form-control form-control-lg" rows="4" path="noidung" placeholder="Ghi chú" />
                    </div>
                    <br>
                    <button class="btn btn-info" data-toggle="modal" data-target="#myModal2">Gửi thông tin</button>
                </form:form>
            </div>
            <div class="col-md-4 col-sm-12 col-xs-12 mt-5">
            	<h3>Số lượng(${soluong})</h3>
            	<br>
            	<div class=" container mt-3">
            		 <c:forEach var="cthd" items="${cthd}">
                    	<div class="media">
    						<img src="resources/hinh/${cthd.hinhanh}" class="align-self-start mr-3" style="width:60px">
    						<div class="media-body">
      							<h8 class="text-center" style="text-transform:uppercase;"><strong>${cthd.tentour}</strong></h8>
      							<p class="text-center"><i>(<fmt:formatDate value="${cthd.ngaydi}" pattern="dd/MM/yyyy" />
      							 - <fmt:formatDate value="${cthd.ngayve}" pattern="dd/MM/yyyy" />) SL/Loại: ${cthd.songuoilon} người lớn, ${cthd.sotreem}trẻ em</i></p>
    						</div>
 					 	</div>
                    </c:forEach>
                     <c:forEach var="ds" items="${dsdv}">
                     	<div class="media">
                     		<img src="resources/hinh/${ds.hinhanh}" class="align-self-start mr-3" style="width:60px">
                     		<div class="media-body">
                     			<h8 class="text-center" style="text-transform:uppercase;"><strong>${ds.tendv}</strong></h8>
                     			<p class="text-center"><i>(<fmt:formatDate value="${ds.ngaynhan}" pattern="dd/MM/yyyy" />
      							 - <fmt:formatDate value="${ds.ngaytra}" pattern="dd/MM/yyyy" />) SL/Loại: ${ds.soluong}/${ds.loai}</i></p>
                     		</div>
                     	</div>
                     </c:forEach>
                    <br>
                    <div><strong>Tổng cộng : <fmt:formatNumber value="${thanhtien}" type="number"/>đ</strong></div>
                     <br>
                     <div><a href="dulichbien/giohang.htm" style="text-decoration:none;"><i class="fas fa-angle-left"></i> Về giỏ hàng</a></div>
                     <br>
                  <a href="dulichbien/tourphuquy.htm" class="text-center"><div class="btn btn-primary">Đặt hàng</div></a>
                 </div>
    ...
    <!-- The Modal -->
     <div class="modal fade" id="myModal2">
        <form action="dulichbien/thanhtoan.htm?xacnhan" method="POST">
		<div class="modal-dialog modal-sm">
         <div class="modal-content">
        	<!-- Modal Header -->
        	<div class="modal-header">
          		<h6 class="modal-title">CẢM ƠN QUÝ KHÁCH ĐÃ SỬ DỤNG DỊCH VỤ CỦA CHÚNG TÔI</h6>
        	</div>
        	<!-- Modal body -->
        	<div class="modal-body">
         		 <p>Chúng tôi sẽ sớm liên hệ với quý khách trong thời gian gần nhất để tiến hành thanh toán dịch vụ.</p>
