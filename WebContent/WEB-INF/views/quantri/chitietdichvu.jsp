  ...
  <a class="navbar-brand"><img src="http://phuquytravel.com.vn/pic/banner/banne_636319247691439271.png" style="width:200px;"></a>
    ...
    <ul class="navbar-nav mr-auto drop-menu">
 	...
 	<div class="container tieude text-info">
    	...
    	<div><p><a href="#">Quản trị</a> <i class="fas fa-angle-right"> chi tiết hóa đơn dịch vụ </i> <i class="fas fa-angle-right"></i> ${message}</p></div>
   		 </div>
	</div>
	...
	<div class="container">
    	<h2>Danh sách hóa đơn dịch vụ du lịch</h2>
  		<p>Bảng hiển thị danh sách các hóa đơn dịch vụ du lịch khách đã đặt của công ty:</p>
        <table class="table" border="1px;">
    		<thead class="thead-dark">
   			 ...
   			 <tbody>
      			<c:forEach var="ctdv" items="${ctdv}">
      				<tr>
      				  <form action="quantri/hieuchinhhoadondv.htm">
      					<td><input class="form-control" name="id" value="${ctdv.ID}" readonly="readonly" type="text"></td>
      					<td>${ctdv.idDV.tenDV}</td>
      					<td>${ctdv.idKH.tenKH}</td>
      					<td><fmt:formatDate value="${ctdv.ngaynhan}" pattern="dd/MM/yyyy" /></td>
      					<td><fmt:formatDate value="${ctdv.ngaytra}" pattern="dd/MM/yyyy" /></td>
      					<td>${ctdv.soluong}</td>
      					<td>${ctdv.loai}</td>
      					<td>
      						<c:if test="${ctdv.thanhtoan=='false'}">Chưa</c:if>
      						<c:if test="${ctdv.thanhtoan=='true'}">Có</c:if>
      					</td>
      					<td><fmt:formatDate value="${ctdv.ngaydat}" pattern="dd/MM/yyyy" /></td>
      					<td>
