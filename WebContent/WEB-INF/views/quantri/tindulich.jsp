...
<body>
  ...
  <a class="navbar-brand"><img src="http://phuquytravel.com.vn/pic/banner/banne_636319247691439271.png" style="width:200px;"></a>
    ...
    <ul class="navbar-nav mr-auto drop-menu">
 	...
 	<div class="container tieude text-info">
		<div class="row pt-4 pl-3">
    		<div><p>Quản trị <i class="fas fa-angle-right"></i> ${tb}</p></div>
    		<div><p>${message}</p></div>
   		 </div>
	</div>
	...
	<div class="container">
    	<h2>Danh sách tin du lịch</h2>
  		<p>Bảng hiển thị danh sách các tin du lịch hiện có của công ty:</p>
        <table class="table" border="1px;">
    		<thead class="thead-dark">
   			 ...
   			 <tbody>
      			<c:forEach var="tdl" items="${tdl}">
      				<tr>
      					<td>${tdl.idTin}</td>
      					<td>${tdl.tieude}</td>
      					<td><img src="resources/hinh/${tdl.hinhanh}" style="width:110px; height:90px;"></td>
      					<td><fmt:formatDate value="${tdl.ngaydang}" pattern="dd/MM/yyyy" /></td>
      					<td>${tdl.noidung}</td>
      					<td>${tdl.idUser.idUser}</td>
      					<td>
      						<form:form action="quantri/themtin.htm" method="GET"><button>Thêm</button></form:form> 
      						<a href="quantri/${tdl.tieude}.htm?Delete" method="GET"><button>Xóa</button></a>
      						<a href="quantri/${tdl.tieude}.htm?Edit" method="GET"><button>Sửa</button></a>  
      					</td>
      				</tr>
      			</c:forEach>
    		</tbody>
