  ...
  <a class="navbar-brand"><img src="http://phuquytravel.com.vn/pic/banner/banne_636319247691439271.png" style="width:200px;"></a>
	...
	<div class="container">
    	<h2>Danh sách tour du lịch</h2>
  		<p>Bảng hiển thị danh sách các tour du lịch hiện có của công ty:</p>
   			 ...
   			 <tbody>
      			<c:forEach var="tdl" items="${tdlich}">
      				<tr>
      					<td>${tdl.idTour}</td>
      					<td>${tdl.tenTour}</td>
      					<td><img style="width:110px; height:90px;" src="resources/hinh/${tdl.hinhanh}"></td>
      					<td>${tdl.khachsan}</td>
      					<td>${tdl.dichuyen}</td>
      					<td>${tdl.thoigian}</td>
      					<td>${tdl.khoihanh}</td>
      					<td><fmt:formatNumber value="${tdl.gia}" type="number"/>đ</td>
      					<td>${tdl.noidung}</td>
      					<td>${tdl.idUser.idUser}</td>
      					<td>
      						<form:form action="quantri/themtour.htm" method="GET"><button>Thêm</button></form:form> 
      						<a href="quantri/${tdl.tenTour}.htm?delete" method="GET"><button>Xóa</button></a>
      						<a href="quantri/${tdl.tenTour}.htm?edit" method="GET"><button>Sửa</button></a> 
      					 </td>
      				</tr>
      			</c:forEach>
    		</tbody>
